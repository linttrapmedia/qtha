...
export async function cleanCommand(_positional: string[], _flags: Record<string, string | boolean>) {
  ...
  const agentPath = resolve(process.cwd(), ".github/agents/qtha.agent.md");
  ...
  const cfgFile = Bun.file(resolve(process.cwd(), "qtha.json"));
  if (await cfgFile.exists()) {
    config.results = defaultConfig().results;
    await writeConfig(config);
    console.log(`  ${green("✓")} Cleared results in ${dim("qtha.json")}`);
  } else {
